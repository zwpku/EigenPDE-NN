...
for idx in range(log_info_vec.shape[1]):
    #plt.clf()
    fig, ax = plt.subplots(1, 1, figsize=(8, 5))
    if idx < len(log_data_name_vec) :
        label_name = log_data_name_vec[idx]
    else :
    ...
    plt.plot(log_info_vec[:,idx], color=lc[idx], label='%s' % label_name)
    ax.legend(bbox_to_anchor=(0.5, 0, 0.5, 0.5))
    plt.yscale('log')
#    ax.set_ylim(-2.1, 2.70)
#    ax.set_yticks(np.arange(-2.1, 2.6, step=0.3))
    fig.tight_layout()
    out_fig_name = '../%s/fig/log_info_%d_%s.eps' % (working_dir_name, idx, label_name)
    fig.savefig(out_fig_name)
    print ("Output plot for %dth column of logfile: %s" % (idx, out_fig_name))
